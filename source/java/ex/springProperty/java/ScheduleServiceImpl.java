 ...
 * @Description : 스케줄 Service 구현 클래스
 * 
 * @author 컨버전스스퀘어
 * @since 2014.08.01
 * @version 1.0
...
public class ScheduleServiceImpl implements ScheduleService {
	...
	@Override
	@Async
	@Scheduled(cron = "${schedule.addSendForBatch.time}")
	...
	@Override
	@Async
	@Scheduled(cron = "${schedule.addSendTarget.time}")
				...
				for (SendMasterVO sendMasterVO : sendMasterVOList) {
					...
					if ("Y".equals(sendMasterVO.getAndYn())) {
						...
						for (DbColumnVO resultDbCoulmnVO : resultDbCoulmnVOList) {
							if ("A".equals(resultDbCoulmnVO.getColReqUse())) {
								andColCd = resultDbCoulmnVO.getColCd();
								break;
							}
						}
						String andRegIdSelectQuery = makeRegIdSelectQuery("A", sendMasterVO.getTableCd(), andColCd, resultCampaignConditionVOList);
						...
						for (String andRegId : andTarRegIdList) {
							if (andRegId != null) {
								...
								temp1SendTargetVO.setAppType("A");
					...
					if ("Y".equals(sendMasterVO.getIosYn())) {
						...
						String iosColCd = "";
						for (DbColumnVO resultDbCoulmnVO : resultDbCoulmnVOList) {
							if ("I".equals(resultDbCoulmnVO.getColReqUse())) {
								iosColCd = resultDbCoulmnVO.getColCd();
								break;
							}
						}
						String iosRegIdSelectQuery = makeRegIdSelectQuery("I", sendMasterVO.getTableCd(), iosColCd, resultCampaignConditionVOList);
						...
						for (String iosRegId : iosTarRegIdList) {
							if (iosRegId != null) {
								...
								temp2SendTargetVO.setAppType("I");
	...
	@Override
	@Async
	@Scheduled(cron = "${schedule.sendMessages.time}")
			...
			try {
				...
				for (SendMasterVO sendMasterVO : sendMasterVOList) {
					...
					for (SendTargetVO sendTargetVO : sendTargetVOList) {
						// 안드로이드 수신자
						if ("A".equals(sendTargetVO.getAppType())) {
							toAndRegIdArray.add(sendTargetVO.getTarRegId());
						}
						// 아이폰 수신자
						else if ("I".equals(sendTargetVO.getAppType())) {
					...
					if (toAndRegIdArray.size() > 0) {
						try {
							// 4.1 GCM 푸시 전송
							JSONObject gcmJsonObj = new JSONObject();
							HttpClientUtil client = new HttpClientUtil("https://gcm-http.googleapis.com/gcm/send", RequestMethod.POST);
							client.addHeader("Content-Type", "application/json; charset=utf-8");
							client.addHeader("Authorization",
									"key=" + ("R".equals(appAndroidVO.getKeyType()) ? appAndroidVO.getKeyRun() : appAndroidVO.getKeyDev()));

							gcmJsonObj.put("registration_ids", toAndRegIdArray);
							gcmJsonObj.put("data", JSONValue.parse(sendMasterVO.getFullMsg()));
							client.setParams(gcmJsonObj);
							client.execute();

							Boolean isSucess = false;
							try {
								if (Integer.parseInt(((JSONObject) JSONValue.parse(client.getRespose())).get("success").toString()) > 0) {
									isSucess = true;
								}
							} catch (Exception e1) {
							}
							...
							LOGGER.info("---------------------> GCM Message Send Result : {} - {}", (isSucess ? "SUCCESS" : "FAIL"),
									String.valueOf(client.getRespose()));
						} catch (Exception e) {
							e.printStackTrace();
						}
					}
					...
					if (toIosRegIdArray.size() > 0) {
						try {
							...
							payload.addCustomAlertBody(sendMasterVO.getFullMsg());

							PushedNotifications notifications = Push.payload(payload,
									PropertyUtil.getProperty("upload", "upload.path.apns.cert") + certFileUrls[certType],
